...
export default function AcademyLogin() {
    ...
    const handleSignIn = async () => {
        ...
        try {
            const res = await axios.post(`${API}/auth/academy-login`, { name: name.trim(), password });
    ...
    const handleSignUp = async () => {
        if (!name.trim() || !password.trim() || !confirmPw.trim()) { setError('All fields are required.'); return; }
        if (password !== confirmPw) { setError('Passwords do not match.'); return; }
        if (password.length < 6) { setError('Password must be at least 6 characters.'); return; }
        setLoading(true); setError('');
        try {
            await axios.post(`${API}/auth/register-academy`, { name: name.trim(), password });
            setSuccess(`Academy "${name.trim()}" created! Signing you in...`);
            setName(''); setPassword(''); setConfirmPw('');
            setTimeout(() => switchTab('signin'), 2000);
        } catch (err) {
            setError(err.response?.data?.detail || 'Could not create academy. Name may already be taken.');
        } finally { setLoading(false); }
    };
    ...
    return (
        ...
        .anim-2 { animation: fadeUp .7s ease .1s both; }
        .anim-3 { animation: fadeUp .7s ease .2s both; }
        .anim-4 { animation: fadeUp .7s ease .3s both; }
        ...
        .card-glow:hover { box-shadow: 0 0 30px rgba(99,102,241,0.08); }
                    ...
                    <div className={`hidden lg:block ${mounted ? '' : 'opacity-0'}`}>
                        ...
                        <div className="anim-4 grid grid-cols-3 gap-4 mb-12">
                            {[
                                ...
                                <div key={s.label} className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 text-center card-glow transition-all">
                        ...
                        <div className="anim-5">
                            ...
                            <div className="space-y-2">
                                {ACADEMIES.map((a, i) => (
                                    <div key={i}
                                        className="flex items-center justify-between bg-white/[0.02] border border-white/5 hover:border-indigo-500/20 hover:bg-indigo-500/[0.03] rounded-xl px-4 py-3 transition-all group">
                                        <div className="flex items-center gap-3">
                                        ...
                                        <span className="flex items-center gap-1.5 text-green-400 text-[11px] font-bold uppercase tracking-wider">
                        ...
                        <div className="bg-white/[0.02] rounded-3xl overflow-hidden backdrop-blur-sm"
                            ...
                            <div className="px-5 sm:px-8 pt-6 sm:pt-8 pb-5 sm:pb-6 border-b border-white/5">
                                ...
                                <p className="text-gray-500 text-sm mt-1">
                                    {tab === 'signin'
                                        ? "Sign in to your academy's private workspace."
                            ...
                            <div className="px-5 sm:px-8 py-6 sm:py-7 space-y-4">
                                ...
                                <p className="text-center text-gray-700 text-xs pt-1">
                                    {tab === 'signin' ? (
                                        <>No academy yet?{' '}
                                            <button onClick={() => switchTab('signup')} className="text-indigo-400 hover:text-indigo-300 font-bold transition">
                                                Register here
                                            </button>
                                        </>
                                    ) : (
                                        <>Already registered?{' '}
                                            <button onClick={() => switchTab('signin')} className="text-indigo-400 hover:text-indigo-300 font-bold transition">
                                                Sign in
                                            </button>
                                        </>
                                    )}
