...
// ── Mental Recharge exercise definitions ──────────────────────────────────────
const ALL_EXERCISES = [
  {
    id: 'box',
    name: 'Box Breathing',
      ...
      <div className="flex items-start justify-between mb-4 gap-3 flex-wrap">
        <div>
          <p className="text-sm font-bold text-gray-300 uppercase tracking-widest">
            🧠 Mind & Mental Recharge
          </p>
          <p className="text-gray-500 text-xs mt-1">
            Guided breathing & visualisation exercises to suggest tonight
          </p>
        </div>
        {!latest && (
      ...
      {latest && (
        <div className="bg-indigo-500/8 border border-indigo-500/15 rounded-xl px-4 py-2.5 mb-4">
          <p className="text-[11px] text-indigo-300 leading-relaxed">
            ...
            soreness <span className="font-black text-white">{latest.soreness}/10</span> — these exercises will
            help your child recover mentally and stay sharp.
          </p>
        </div>
      )}
        ...
        {suggestions.map((ex) => (
          <div key={ex.id} className="bg-gray-700/40 rounded-xl p-4 border border-gray-600/30">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest"
              ...
              Best used when:{' '}
              <span className="normal-case font-normal text-gray-400">{ex.when}</span>
            </p>
          </div>
        ))}
      </div>
      ...
      {expanded && (
        <div className="flex flex-col gap-2 mt-3">
          {ALL_EXERCISES.filter(ex => !suggestions.find(s => s.id === ex.id)).map(ex => (
            <div key={ex.id}
              className="flex items-center gap-3 bg-gray-700/20 rounded-xl px-4 py-3 border border-gray-600/20">
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 md:mb-10">
          <div>
            <h1 className="text-2xl md:text-4xl font-black tracking-tight">AthleteIQ</h1>
            <p className="text-gray-400 text-xs md:text-sm mt-1">Parent View · Live Updates</p>
          </div>
          <button onClick={handleLogout}
            className="border border-gray-600 text-gray-400 px-3 py-2 rounded-xl text-xs md:text-sm hover:border-red-500 hover:text-red-400 transition">
            Logout
          </button>
        </div>

        {/* Child header card */}
        <div className="bg-purple-500/10 border border-purple-500/20 rounded-2xl p-5 mb-5 flex justify-between items-center">
          <div>
            <p className="text-purple-400 text-xs uppercase tracking-widest font-bold mb-1">Viewing Progress For</p>
            <h2 className="text-2xl md:text-3xl font-black text-white">{childName}</h2>
          </div>
          <div className="flex items-center gap-3">
            {insight?.score != null && (
              <div className="text-center bg-gray-800 rounded-2xl px-4 py-2 border border-gray-700">
                <p className="text-3xl md:text-4xl font-black text-white">{insight.score}</p>
              ...
              {recovery.coach_message && (
                <div className={`rounded-xl p-4 border mt-2 ${recovery.risk_level === 'red' ? 'bg-red-500/10 border-red-500/20'
                  ...
                  <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${recovery.risk_level === 'red' ? 'text-red-400'
                      : recovery.risk_level === 'yellow' ? 'text-yellow-400'
                        : 'text-green-400'
                    }`}>Coach's Message to Parents</p>
          ...
          <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700 mb-5">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={history}>
                ...
                <YAxis domain={[0, 10]} tick={{ fill: '#9ca3af', fontSize: 11 }} axisLine={{ stroke: '#374151' }} />
                ...
                <Line type="monotone" dataKey="sleep" stroke="#a855f7" strokeWidth={2} dot={{ r: 3 }} name="Sleep" />
                <Line type="monotone" dataKey="soreness" stroke="#ef4444" strokeWidth={2} dot={{ r: 3 }} name="Soreness" />
                <Line type="monotone" dataKey="mood" stroke="#eab308" strokeWidth={2} dot={{ r: 3 }} name="Mood" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
            ...
            <div className="flex flex-col gap-3">
              {[...history].reverse().map((entry, i) => (
                <div key={i} className="flex gap-3 items-start">
                  <div className="text-gray-500 text-xs w-12 pt-2 shrink-0">{entry.date}</div>
                  <div className="flex-1 bg-gray-700/50 rounded-xl p-4 border border-gray-600/50">
                    <div className="grid grid-cols-4 gap-2 mb-2">
                      {['energy', 'sleep', 'soreness', 'mood'].map(key => (
                        <div key={key} className="text-center">
                          <p className={`font-black text-lg ${key === 'energy' ? 'text-blue-400'
